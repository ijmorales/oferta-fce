...
import { FaArrowCircleUp, FaArrowCircleDown, FaChevronCircleDown } from 'react-icons/fa'
...
export const SORT_OPTIONS = [
  {
    ...
    sortFn: function (direction) {
      if (direction === 'asc') {
        return (a, b) => a.materia.toLowerCase().localeCompare(b.materia.toLowerCase())
      } else {
        return (a, b) => b.materia.toLowerCase().localeCompare(a.materia.toLowerCase())
      }
    }
  },
  {
...
export default function SortBy ({ handleSort, handleSortDirection, currentSort, currentSortDirection }) {
  const clickHandler = (displayName) => {
    const sortObj = SORT_OPTIONS.find((option) => option.displayName === displayName)
    handleSort(sortObj)
  }
      ...
      >
        <div className="flex items-center">
          <span className="mr-3 flex items-center">
            <p className="mr-2">
              Ordenar por: { currentSort.displayName }
            </p>
          <FaChevronCircleDown/>
          </span>
        </div>
      </Dropdown>
      <div onClick={() => handleSortDirection(currentSortDirection === 'asc' ? 'desc' : 'asc')}>
        {currentSortDirection === 'asc' ? <FaArrowCircleUp/> : <FaArrowCircleDown/>}
      </div>
    </>
  )
}
